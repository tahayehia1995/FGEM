...
class HighEnthalpyCLGWGPowerPlant(BasePowerPlant):
    ...
    def __init__(self, Tres, Tamb, m_prd, num_prd, cf=1.0, high_resolution=False, k=2):
        ...
        # Validate inputs before model.predict (sklearn will often throw low-level NaN/Inf errors)
        model_input = np.array([[self.Tamb_design, self.Tres_design, self.m_prd_design]], dtype=float)
        if not np.all(np.isfinite(model_input)):
            raise ValueError(
                "HighEnthalpyCLGWGPowerPlant init received non-finite inputs "
                f"(Tamb={self.Tamb_design}, Tres={self.Tres_design}, m_prd={self.m_prd_design})."
            )
    ...
    def compute_geofluid_consumption(self, T, T_amb, m_turbine):
        ...
        # Validate inputs early so errors are actionable
        model_input = np.array([[T_amb, T, m_prd]], dtype=float)
        if not np.all(np.isfinite(model_input)):
            raise ValueError(
                "HighEnthalpyCLGWGPowerPlant received non-finite inputs to predict(): "
                f"T_amb={T_amb}, T={T}, m_prd={m_prd} (from m_turbine={m_turbine}, num_prd={self.num_prd})."
            )
