...
class World:
    ...
    def __init__(self, config, reset_market_weather=True, config_manager=None):
        """Defining attributes for the World class.

        Args:
            config (dict): json object with project configuration.
            reset_market_weather (bool): whether to reset market and weather data.
            config_manager: Optional ConfigManager instance. If None, uses defaults from set_defaults() for backward compatibility.
        """
        
        # Handle config manager or backward compatibility
        try:
            if config_manager is not None:
                # Use ConfigManager to merge config with defaults
                from ..core.config_manager import ConfigManager
                if isinstance(config_manager, ConfigManager):
                    merged_config = config_manager.merge_config(config)
                else:
                    # Assume it's a dict and create ConfigManager
                    cm = ConfigManager()
                    merged_config = cm.merge_config(config)
                self.config = merged_config
            else:
                # Backward compatibility: use old set_defaults() method
                self.set_defaults()
                self.config = config
        except ImportError:
            # If streamlit_app is not available, use backward compatibility mode
            self.set_defaults()
            self.config = config
        ...
        self.PumpingPower = 0
        # Initialize m_inj based on m_prd and inj_prd_ratio (if m_prd exists)
        if hasattr(self, 'm_prd') and self.m_prd is not None:
            self.m_inj = self.m_prd * self.inj_prd_ratio if hasattr(self, 'inj_prd_ratio') else self.m_prd
        else:
            self.m_inj = None
    ...
    def step_update_record(self,
        ...
        if self.reservoir_filename:
            ...
            # Calculate m_inj from m_prd and inj_prd_ratio if m_inj not initialized
            if self.m_inj is None:
                m_inj = m_prd * self.inj_prd_ratio
            else:
    ...
    def update_state(self, 
        ...
        def _to_float(x):
            """Best-effort scalar conversion (returns np.nan on failure)."""
        ...
        # Prefer explicit None checks (np.nan is truthy and should NOT override defaults)
        T_amb_val = self.state["T0"] if T_amb is None else T_amb
        T_amb_val = _to_float(T_amb_val)
        if not _is_finite_number(T_amb_val):
            fallback = _to_float(self.state.get("T0", self.surface_temp))
            T_amb_val = fallback if _is_finite_number(fallback) else float(self.surface_temp)
        self.T_amb = float(T_amb_val)
        ...
        self.m_prd = np.array(m_prd, dtype=float) if isinstance(m_prd, (list, tuple, np.ndarray)) else m_prd
        self.m_inj = np.array(m_inj, dtype=float) if isinstance(m_inj, (list, tuple, np.ndarray)) else m_inj

        # Ensure scalar values per reservoir (avoid dtype=object arrays when reservoirs store per-well arrays)
        self.res_shutoffs = np.array([bool(getattr(r, "shutoff", False)) for r in self.reservoirs], dtype=float)
        self.res_bh_temps = np.array([_to_float(getattr(r, "Tres", np.nan)) for r in self.reservoirs], dtype=float)
        self.res_wh_temps = np.array([_to_float(getattr(r, "T_prd_wh", np.nan)) for r in self.reservoirs], dtype=float)
        ...
        weights = self.res_total_prd * self.C_res
        valid = np.isfinite(self.res_wh_temps) & np.isfinite(weights) & (weights > 0)
        if np.any(valid):
            denom = float(np.sum(weights[valid]))
            numer = float(np.sum(weights[valid] * self.res_wh_temps[valid]))
            self.T_prd_wh = numer / denom if denom > 0 else float(np.nanmean(self.res_wh_temps[valid]))
        else:
            # Fallback: try finite reservoir BH temps, otherwise Tres_init
            if np.any(np.isfinite(self.res_bh_temps)):
                self.T_prd_wh = float(np.nanmean(self.res_bh_temps))
            else:
                self.T_prd_wh = float(self.Tres_init)

        if not _is_finite_number(self.T_prd_wh):
            # Last resort: keep a finite value to avoid poisoning downstream economics
            self.T_prd_wh = float(self.Tres_init)

        self.m_g = float(np.nansum(self.res_total_prd))

        self.m_bypass = m_bypass
        self.m_excess = 0
        self.m_turbine = float(self.m_g + self.m_tes_out - self.m_tes_in - self.m_bypass)
        if not _is_finite_number(self.m_turbine):
            self.m_turbine = 0.0
        self.m_turbine = max(self.m_turbine, 0.0)
        ...
        T_inj_val = self.powerplant.T_inj if T_inj is None else T_inj
        T_inj_val = _to_float(T_inj_val)
        if not _is_finite_number(T_inj_val):
            fallback = _to_float(getattr(self.powerplant, "T_inj", np.nan))
            T_inj_val = fallback if _is_finite_number(fallback) else float(self.T_prd_wh)
        self.T_inj = float(T_inj_val)

    def step(self):
        """Stepping the project in time.
        """
        # Ensure T_prd_wh is set (should be set by update_state, but check here for safety)
        if not hasattr(self, 'T_prd_wh') or self.T_prd_wh is None:
            # Initialize from reservoirs if not set
            if self.reservoirs:
                r = self.reservoirs[0]
                if hasattr(r, 'T_prd_wh'):
                    if isinstance(r.T_prd_wh, (np.ndarray, list)):
                        self.T_prd_wh = float(np.mean(r.T_prd_wh))
                    else:
                        self.T_prd_wh = float(r.T_prd_wh)
                else:
                    self.T_prd_wh = float(r.Tres) if hasattr(r, 'Tres') else 225.0
            else:
                self.T_prd_wh = float(self.Tres_init) if hasattr(self, 'Tres_init') else 225.0
        
        # Guardrail: never feed NaN/Inf into powerplant logic (prevents obscure NumPy/SciPy validation errors)
        if not np.isfinite(self.T_prd_wh):
            self.T_prd_wh = float(self.Tres_init)
        if not np.isfinite(self.T_amb):
            self.T_amb = float(self.surface_temp)
        if not np.isfinite(self.m_turbine):
            self.m_turbine = 0.0
        self.m_turbine = max(float(self.m_turbine), 0.0)
    ...
    def compute_pumping(self):
        ...
        for reservoir in self.reservoirs:
            ...
            if self.reservoir_filename:
                ...
                reservoir = self.reservoirs[0]  # Get the first reservoir from the list
                T_inj = T_inj[:,None]
                diam = 2*reservoir.radiusvector[None]
                m = m_inj[:,None]
                ...
                # Handle broadcasting: arrays might be 2D (timesteps x wells) or 1D (timesteps)
                # Convert to 1D by summing/averaging across wells dimension
                if m_prd.ndim > 1:
                    m_prd = np.sum(m_prd, axis=1)  # Sum across wells for total production
                if m_inj.ndim > 1:
                    m_inj = np.sum(m_inj, axis=1)  # Sum across wells for total injection
                if T_prd_wh.ndim > 1:
                    T_prd_wh_avg = np.mean(T_prd_wh, axis=1)  # Average temperature across wells
                else:
                    T_prd_wh_avg = T_prd_wh
                # Ensure Tres is 1D and matches length
                if Tres.ndim > 1:
                    Tres = Tres.flatten()[:len(T_prd_wh_avg)]
                elif len(Tres) != len(T_prd_wh_avg):
                    Tres = Tres[:len(T_prd_wh_avg)]
                Tavg = (3/4*Tres+1/4*T_prd_wh_avg) #most of temperature drop happens in upper section (because surrounding rock temperature is lowest in upper section)
                ...
                # Handle T_prd_wh if it's 2D (average across wells)
                if isinstance(T_prd_wh, np.ndarray) and T_prd_wh.ndim > 1:
                    T_prd_wh_for_vapor = np.mean(T_prd_wh, axis=1)
                else:
                    T_prd_wh_for_vapor = T_prd_wh
                self.Pprodwellhead = vaporpressurewater(T_prd_wh_for_vapor) + Pexcess #[kPa] is minimum production pump inlet pressure and minimum wellhead pressure
                ...
                # Ensure m_prd is 1D (sum across wells if 2D)
                if m_prd.ndim > 1:
                    m_prd_total = np.sum(m_prd, axis=1)
                else:
                    m_prd_total = m_prd
                self.pumpdepth = reservoir.well_tvd + (self.Pprodwellhead - self.Phydrostatic + m_prd_total/self.PIkPa)/(self.f3*(self.rhowaterprod*self.vprod**2/2.)*(1/reservoir.prd_well_diam)/1E3 + self.rhowaterprod*9.81/1E3)
                ...
                self.DP3 = self.Pprodwellhead - (self.Phydrostatic - m_prd_total/self.PIkPa - self.rhowaterprod*9.81*reservoir.well_tvd/1E3 - self.f3*(self.rhowaterprod*self.vprod**2/2.)*(reservoir.well_md/reservoir.prd_well_diam)/1E3)
                PumpingPowerProd = self.DP3*m_prd_total*num_prd/self.rhowaterprod/self.pumpeff/1e3 #[MWe] total pumping power for production wells
    ...
    def compute_economics(self, print_outputs=True):
        ...
        # Use a finite median (NaNs here can propagate into capex/opex and later crash IRR)
        try:
            wh_series = pd.to_numeric(self.df_records.get("WH Temp [deg C]"), errors="coerce")
            wh_med = float(np.nanmedian(wh_series.values)) if wh_series is not None else np.nan
        except Exception:
            wh_med = np.nan
        if not np.isfinite(wh_med):
            wh_med = float(getattr(self, "Tres_pp_design", self.Tres_init))
        self.Cplant = self.powerplant.compute_cplant(wh_med, min_cost=self.powerplant_usd_per_kw_min)
        ...
        # Note: Drilling cost calculation is consistent across all reservoir types using the same
        # compute_drilling_cost() function. The only difference is well geometry:
        # - uloop reservoir type uses half_lateral_length (closed-loop geometry)
        # - All other reservoir types use lateral_length (open-loop geometry)
        # This ensures consistent cost calculation methodology regardless of reservoir type.
        ...
        # Pumping Cost
        inj_power_mwe = pd.to_numeric(self.df_records["Injection Pumping Power [MWe]"], errors="coerce").values
        injpumphp = float(np.nanmax(inj_power_mwe)) * 1341 if inj_power_mwe.size else 0.0
        if (not np.isfinite(injpumphp)) or injpumphp <= 0:
            Cpumpsinj = 0.0
        else:
            numberofinjpumps = float(np.ceil(injpumphp / 2000.0))  # pump can be maximum 2,000 hp
            numberofinjpumps = max(numberofinjpumps, 1.0)
        ...
        prod_power_mwe = pd.to_numeric(self.df_records["Production Pumping Power [MWe]"], errors="coerce").values
        prodpumphp = float(np.nanmax(prod_power_mwe)) * 1341 if prod_power_mwe.size else 0.0  # hp

        # Protect against NaN propagation (np.max with NaNs => NaN; NaN * 0 is still NaN)
        if (not np.isfinite(prodpumphp)) or prodpumphp <= 0:
            Cpumpsprod = 0.0
        else:
            pumpdepth = getattr(self, "pumpdepth", 0.0)
            try:
                pumpdepth_ft = float(np.nanmax(np.asarray(pumpdepth, dtype=float))) * 3.281
            except Exception:
                pumpdepth_ft = 0.0
            if not np.isfinite(pumpdepth_ft):
                pumpdepth_ft = 0.0

            Cpumpsprod = float(
                (1.5 * (1750 * (prodpumphp) ** 0.7 + 5750 * (prodpumphp) ** 0.2 + 10000 + pumpdepth_ft * 50))
            )  # see page 46 in user's manual assuming rental of rig for 1 day.
        ...
        # Contingency should not become NaN if any upstream capex term is NaN
        capex_stack = np.array([np.asarray(v, dtype=float) for v in self.capex.values()], dtype=float)
        self.capex["Contingency"] = self.contingency * np.nan_to_num(np.nansum(capex_stack, axis=0), nan=0.0, posinf=0.0, neginf=0.0)  # Based on GETEM page 21
        ...
        # Total CAPEX/OPEX
        self.capex_total = np.nan_to_num(np.sum(np.array([v for v in self.capex.values()]), axis=0), nan=0.0, posinf=0.0, neginf=0.0) * (1 - self.itc)
        self.opex_total = np.nan_to_num(np.sum(np.array([v for v in self.opex.values()]), axis=0), nan=0.0, posinf=0.0, neginf=0.0) # $MM/year
    ...
    def create_powerplant(self):
       # specify what production temperature the power plant should be designed for ...
        ...
        if "geophires" in self.powerplant_type.lower():
            ...
            # Get ambient temperature - handle both Series and scalar values
            # df_market should exist at this point, but handle edge cases
            if hasattr(self, 'df_market') and self.df_market is not None and isinstance(self.df_market, pd.DataFrame):
                if 'T0' in self.df_market.columns:
                    T0_col = self.df_market['T0']
                    if isinstance(T0_col, pd.Series) and len(T0_col) > 0:
                        Tamb = float(T0_col.mean())
                    elif hasattr(T0_col, 'iloc'):
                        Tamb = float(T0_col.iloc[0]) if len(T0_col) > 0 else self.surface_temp
                    else:
                        Tamb = float(T0_col) if T0_col is not None else self.surface_temp
                else:
                    Tamb = self.surface_temp
            else:
                Tamb = self.surface_temp
            
            powerplant = ORCPowerPlant(ppc=self.powerplant_capacity, Tres=self.Tres_pp_design, Tamb=Tamb, 
    ...
    def create_reservoir(self, num_prd, num_inj, time_init):
       # if reservoir filepath is provided, then override reservoir object to use the provided tabular data
        if self.reservoir_filepath:
            ramey_val = getattr(self, 'ramey', True) if getattr(self, 'ramey', None) is not None else True
            pumping_val = getattr(self, 'pumping', True)
            krock_val = getattr(self, 'krock', 3.0)
            rhorock_val = getattr(self, 'rhorock', 2700.0)
            cprock_val = getattr(self, 'cprock', 1000.0)
            impedance_val = getattr(self, 'impedance', 0.1)
            N_ramey_mv_avg_val = getattr(self, 'N_ramey_mv_avg', 168)
            krock_wellbore_val = getattr(self, 'krock_wellbore', 3.0)
            
            # Convert timestep_hours to timedelta if provided
            from datetime import timedelta
            timestep_val = timedelta(hours=getattr(self, 'timestep_hours', 1.0))
            ...
            reservoir = TabularReservoir(Tres_init=self.Tres_init, geothermal_gradient=self.geothermal_gradient, surface_temp=self.surface_temp, 
                            ...
                            drawdp=self.drawdp, plateau_length=self.plateau_length, reservoir_simulator_settings=self.reservoir_simulator_settings, 
                            ramey=ramey_val, PumpingModel=self.PumpingModel, pumping=pumping_val, krock=krock_val, rhorock=rhorock_val, 
                            cprock=cprock_val, impedance=impedance_val, N_ramey_mv_avg=N_ramey_mv_avg_val,
                            timestep=timestep_val, krock_wellbore=krock_wellbore_val,
            ...
            if self.reservoir_type == "energy_decline":
                ...
                cprock_val = getattr(self, 'cprock', 1000.0)
                impedance_val = getattr(self, 'impedance', 0.1)
                N_ramey_mv_avg_val = getattr(self, 'N_ramey_mv_avg', 24)  # EnergyDeclineReservoir default is 24
                krock_wellbore_val = getattr(self, 'krock_wellbore', 3.0)
                ...
                reservoir = EnergyDeclineReservoir(Tres_init=self.Tres_init, Pres_init=self.Pres_init, geothermal_gradient=self.geothermal_gradient, surface_temp=self.surface_temp, 
                                            ...
                                            rock_energy_recovery=self.rock_energy_recovery, reservoir_simulator_settings=self.reservoir_simulator_settings, 
                                            ramey=ramey_val, PumpingModel=self.PumpingModel, pumping=pumping_val, krock=krock_val, rhorock=rhorock_val, 
                                            cprock=cprock_val, impedance=impedance_val, N_ramey_mv_avg=N_ramey_mv_avg_val, 
                                            timestep=timestep_val, krock_wellbore=krock_wellbore_val)

            elif self.reservoir_type == "diffusion_convection":                
                # Use diffusion-specific rock properties if available, otherwise use general krock/cprock
                krock_val = getattr(self, 'krock_diffusion', None) or self.krock
                rhorock_val = getattr(self, 'rhorock_diffusion', None) or getattr(self, 'rhorock', 2600.0)
                cprock_val = getattr(self, 'cprock_diffusion', None) or self.cprock
                ramey_val = getattr(self, 'ramey', True) if getattr(self, 'ramey', None) is not None else True
                pumping_val = getattr(self, 'pumping', True)
                krock_wellbore_val = getattr(self, 'krock_wellbore', 3.0)
                ...
                reservoir = DiffusionConvection(Tres_init=self.Tres_init, geothermal_gradient=self.geothermal_gradient, surface_temp=self.surface_temp,
                                            ...
                                            krock=krock_val, rhorock=rhorock_val, cprock=cprock_val, 
                                            reservoir_simulator_settings=self.reservoir_simulator_settings, PumpingModel=self.PumpingModel, 
                                            ramey=ramey_val, pumping=pumping_val, timestep=timestep_val, krock_wellbore=krock_wellbore_val)

            elif self.reservoir_type == "uloop":
                # Use ULoop-specific simulator settings from config
                k_m_val = getattr(self, 'k_m', None) or self.krock
                rho_m_val = getattr(self, 'rho_m', 2875.0)
                c_m_val = getattr(self, 'c_m', 825.0)
                k_f_val = getattr(self, 'k_f', 0.68)
                mu_f_val = getattr(self, 'mu_f', 600e-6)
                cp_f_val = getattr(self, 'cp_f', 4200.0)
                rho_f_val = getattr(self, 'rho_f', 1000.0)
                fullyimplicit_val = getattr(self, 'fullyimplicit', 1)
                FMM_val = getattr(self, 'FMM', 1)
                FMMtriggertime_val = getattr(self, 'FMMtriggertime', 864000.0)
                ramey_val = getattr(self, 'ramey', False) if getattr(self, 'ramey', None) is not None else False
                pumping_val = getattr(self, 'pumping', True)
                ...
                reservoir = ULoopSBT(Tres_init=self.Tres_init, Pres_init=self.Pres_init, surface_temp=self.surface_temp, geothermal_gradient=self.geothermal_gradient,
                            ...
                            times_arr=np.linspace(0, self.max_simulation_steps, self.max_simulation_steps), 
                            reservoir_simulator_settings=self.reservoir_simulator_settings, PumpingModel=self.PumpingModel,
                            closedloop_design=self.closedloop_design, ramey=ramey_val, pumping=pumping_val, dx=self.dx, 
                            k_m=k_m_val, rho_m=rho_m_val, c_m=c_m_val, k_f=k_f_val, mu_f=mu_f_val, cp_f=cp_f_val, rho_f=rho_f_val,
                            fullyimplicit=fullyimplicit_val, FMM=FMM_val, FMMtriggertime=FMMtriggertime_val)
                ...
                # Use Coaxial-specific simulator settings from config
                # Note: CoaxialSBT doesn't accept ramey/pumping as parameters - they're hardcoded in super() call
                k_m_val = getattr(self, 'k_m', None) or self.krock
                rho_m_val = getattr(self, 'rho_m', 2875.0)
                c_m_val = getattr(self, 'c_m', 825.0)
                k_f_val = getattr(self, 'k_f', 0.6)
                mu_f_val = getattr(self, 'mu_f', 600e-6)
                cp_f_val = getattr(self, 'cp_f', 4184.0)
                rho_f_val = getattr(self, 'rho_f', 990.0)
                FMM_val = getattr(self, 'FMM', 1)
                FMMtriggertime_val = getattr(self, 'FMMtriggertime', 864000.0)
                ...
                reservoir = CoaxialSBT(Tres_init=self.Tres_init, Pres_init=self.Pres_init, surface_temp=self.surface_temp, geothermal_gradient=self.geothermal_gradient,
                                            ...
                                            dx=self.dx, k_m=k_m_val, rho_m=rho_m_val, c_m=c_m_val, 
                                            k_f=k_f_val, mu_f=mu_f_val, cp_f=cp_f_val, rho_f=rho_f_val,
                                            FMM=FMM_val, FMMtriggertime=FMMtriggertime_val)
                ...
                reservoir = PercentageReservoir(Tres_init=self.Tres_init, geothermal_gradient=self.geothermal_gradient, surface_temp=self.surface_temp, 
                                ...
                                num_inj=self.num_inj, waterloss=self.waterloss,
                                powerplant_type=self.powerplant_type, pumpeff=self.pumpeff, PI=self.PI, II=self.II, SSR=self.SSR, 
                                drawdp=self.drawdp, plateau_length=self.plateau_length, 
                                reservoir_simulator_settings=self.reservoir_simulator_settings, ramey=ramey_val, PumpingModel=self.PumpingModel,
                                pumping=pumping_val, krock=krock_val, rhorock=rhorock_val, cprock=cprock_val,
                                impedance=impedance_val, N_ramey_mv_avg=N_ramey_mv_avg_val, 
                                timestep=timestep_val, krock_wellbore=krock_wellbore_val)
    ...
    def compute_npv(self, ppa_price=None, ppa_escalaction_rate=None, print_outputs=True):
        ...
        cashflow = np.nan_to_num(self.df_annual_nominal["Cashflow [$MM]"].values, nan=0.0, posinf=0.0, neginf=0.0)
        try:
            self.IRR = float(npf.irr(cashflow) * 100)
        except Exception:
            self.IRR = np.nan
        ...
        ppa_cashflow = np.nan_to_num(self.df_annual_nominal["PPA Cashflow [$MM]"].values, nan=0.0, posinf=0.0, neginf=0.0)
        try:
            self.PPA_IRR = float(npf.irr(ppa_cashflow) * 100)
        except Exception:
            self.PPA_IRR = np.nan
